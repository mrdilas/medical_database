<template>
  <div class="window"> 
    ...
    <div v-if="diagnoses.length === 0">
      <h1>
        Не найдено диагнозов.
      </h1>
    </div>

    <!-- Вывод диагнозов -->
    <div v-for="diagnosis in diagnoses" :key="diagnosis.id" class="card_patient">
      <div class="card_button">
        <h1>{{ diagnosis.diseases_name }}</h1> <!-- Используем diagnosis.diseases_name -->

        <button class="icon-button" @click="deleteDiagnosis(diagnosis.id)">
    ...
    <button @click="openAddingDiagnosis">
        Добавить диагноз
    </button>
...
export default{
  ...
  data() {
      return {
          diagnoses: [], // Массив для хранения диагнозов
      }
  },
  mounted() {
      this.fetchDiagnoses(); // Загружаем диагнозы при монтировании компонента
  },
  
  methods: {
      async fetchDiagnoses() {
          try {
              const response = await axios.get('http://localhost:3000/api/diseases'); // Запрос к API
              this.diagnoses = response.data;
          } catch (error) {
              console.error("Ошибка при получении диагнозов:", error);
              alert("Не удалось загрузить диагнозы. Пожалуйста, попробуйте позже."); // Рекомендуется уведомить пользователя
          }
      },
      
      async deleteDiagnosis(id) {
          // Запрашиваем подтверждение удаления
          const confirmDelete = confirm("Вы уверены, что хотите удалить этот диагноз?");
          if (!confirmDelete) {
          ...
          try {
              ...
              await axios.delete(`http://localhost:3000/api/diseases/${id}`);
              ...
              alert("Не удалось удалить диагноз. Числится в истории болезни."); 
          }
      },

      openAddingDiagnosis() {
          this.$emit('changePage', 'addingDiagnosis');
      },
